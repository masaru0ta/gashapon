...
最大高さ180px、背景色 `#1a1a2e`。3カラム構成。画面が小さい場合は自動的に縮小される（レスポンシブ対応参照）。
...
- ステータスバーは `height: auto; max-height: 180px` とし、内容量に応じた高さで、最大180pxまでとする
...
| 要素 | 最小値 | 推奨値 | 最大値 |
|------|--------|--------|--------|
| ユニット数 | 20px | 5.5vh | 42px |
| ターン番号 | 16px | 3.5vh | 28px |
| フェーズ | 14px | 3vh | 24px |
| IG/CT | 12px | 2.3vh | 18px |
| その他（GP/GB/T） | 10px | 2vh | 16px |

**制約**:
- ステータスバーの全要素が、あらゆるビューポートサイズで画面内に収まること（はみ出し不可）
- `overflow: hidden` は維持する
- PC表示（高さ800px以上）ではステータスバーの高さが概ね従来の180px相当となること
