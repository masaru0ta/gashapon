...
画面高さが小さいデバイス（スマートフォン等）では、ステータスバーが画面外にはみ出さないようにCSSで自動縮小する。

| 条件 | ステータスバー高さ | ユニット数フォント | ターン番号フォント | IG/CTフォント | その他フォント | パディング |
|------|-------------------|-------------------|-------------------|--------------|--------------|-----------|
| 画面高さ 700px 超 | 180px | 42px | 28px | 18px | 16px | 8px 16px |
| 画面高さ 700px 以下 | 120px | 28px | 20px | 14px | 12px | 4px 8px |

- 全てのステータスバー要素が画面の最下部に収まること（はみ出し不可）
- ステータスバーの `overflow: hidden` は維持する
- ミニマップのサイズも画面高さ 700px 以下で縮小する（幅150px、高さ90px）
- 中央パネルの幅も画面高さ 700px 以下で縮小する（200px）
...
- 画面高さ667px（スマホサイズ）でステータスバーの全要素がビューポート内に収まる
- 画面高さ667pxでP1・P2のユニット総数が画面内に表示される
- 画面高さ667pxでミニマップが画面内に表示される
- 画面高さ800px（PC）ではステータスバーの高さが180pxである
